...
{target:`&+ndsp;`, content:`半角スペースを挿入（+除く）。<br>&lt;p&gt;This is***some text.&lt;/p&gt;`},
{target:`&+ensp;`, content:`通常のスペースとは異なり、幅が広いスペースを挿入（+除く）。<br>&lt;p&gt;This is***some text.&lt;/p&gt;`},
{target:`&+emsp;`, content:`通常のスペースとは異なり、半角2倍のスペースを挿入（+除く）。<br>&lt;p&gt;This is***some text.&lt;/p&gt;`},
{target:`&+nbsp;`, content:`ノーブレークスペース（前後で自動改行しないスペース）(+除く)。<br>&lt;p&gt;This is***some text.&lt;/p&gt;`},
{target:`&+lt;`, content:`小なり記号（+除く）。`},
{target:`&+gt;`, content:`大なり記号（+除く）。`},
{target:`&+copy;`, content:`コピーライト記号（+除く）。`},
...
{target:`flex-direction`, content:`レイアウトの方向のCSSプロパティ。<br>***: row;&thinsp***: row-reverse;&thinsp***: column;`},
...
{target:`justify-content`, content:`レイアウトの方向に沿っての配置するCSSプロパティ。<br>***: center;&thinsp: space-between;&thinsp***: space-around;&thinsp***: space-evenly; `},
...
{target:`align-items`, content:`レイアウト方角に直角に配置するCSSプロパティ。center|flex=enc|space-around\space-between|stretch。<br>***: stretch;&thinsp***: center;&thinsp***: start;&thinsp***: end;`},
...
{target:`transform`, content:`要素の変形（位置、回転、スケールなど）を指定するCSSプロパティ。<br>***: rotate(0.5turn);&thinsp***: scale(2, 0.5);&thinsp***: scale(0.5) translate(-100%, -100%);`},
...
{target:`linear-gradient`, content:`二つ以上の色の連続的な直線に沿った変化から構成される画像を生成するCSS関数。<br>background: ***(to right, #FFD700, #FF4500);`},
...
{target:`unicode-range`, content:`@font-faceで定義されたフォントから使用される特定の文字範囲を設定し、現在のページで使用できるようにするCSS記述子。<br>@font-face { ***: U+0025-00FF; }`},
...
{target:`&lt;fieldset&gt;`, content:`フォーム内の関連要素のグルーピングする。<br>&lt;***&gt;&lt;legend&gt;Group&lt;/legend&gt;...&lt;/***&gt;`},
