...
{target:`style`, content:`要素のインラインスタイルをCSS Style Declarationオブジェクトの形で返すHTML Elementの読み取り専用プロパティ。このオブジェクトは、その要素のインラインのstyle属性で定義されている属性に値が割り当てられ、その要素のすべてのスタイルプロパティのリストが入っている。スタイル宣言は、null または空文字列を設定することでリセットできる。<br>&lt;body ***='font-weight:bold'&gt;<br>element.***.backgroundColor = 'red'<br>element.***.cssText = 'width: 300px; position: relative; top: 250px; left: 250px';`},
...
{target:`class`, content:`複数の要素に同じスタイルを適用するためのクラス。スペースで区切ることで複数のクラスを指定可能。<br>&lt;p ***='highlight'&gt;This is highlighted text.&lt;/p&gt;<br>&lt;p ***='highlight wrapper'&gt;This is highlighted text and wrapper.&lt;/p&gt; → highlightとwrapperの2つのクラスを指定`},
...
{target:`button`, content:`プッシュボタンで、既定の動作を持たず、value 属性の値（既定では空）を表示するtype属性で指定するinputの型。<br>&lt;input class='styled' type='***' value='Add to favorites' /&gt;`},
...
{target:`color`, content:`色を指定するためのコントロールのtype属性で指定するinputの型。。対応しているブラウザーでは、アクティブになったときにカラーピッカーが開く。<br>&lt;p&gt;カラー選択&lt;/p&gt;<br>&lt;div&gt;<br>&emsp;&lt;input type='***' id='head' name='red' value='#e66465' /&gt;<br>&emsp;&lt;label for='red'&gt;Red&lt;/label&gt;<br>&lt;/div&gt;<br>&lt;div&gt;<br>&emsp;&lt;input type='***' id='yellow' name='yellow' value='#f6b73c' /&gt;<br>&emsp;&lt;label for='yellow'&gt;Yellow&lt;/label&gt;<br>&lt;/div&gt;`},
{target:`date`, content:`日付（時刻を除いた年、月、日）を入力するためのコントロールのtype属性で指定するinputの型。。対応しているブラウザーでは、アクティブになったときに日付ピッカーまたは年月日の数値のホイールが開く。<br>&lt;input type='***' id='start' name='trip-start' value='2024-01-01' min='2024-01-01' max='2024-12-31' /&gt;`},
{target:`dateteime-local`, content:`タイムゾーン情報がない日付と時刻を入力するためのコントロールのtype属性で指定するinputの型。。対応しているブラウザーでは、アクティブになったときに日付ピッカーまたは日付および時刻部分の数値のホイールが開く。<br>&lt;input type='***' id='meeting-time' name='meeting-time' value='2024-01-01T00:00' min='2024-01-01T00:00' max='2024-12-31T23:59' /&gt;`},
{target:`email`, content:`ユーザーに一つのメールアドレス、または、 multiple 属性が設定されていた場合は、メールアドレスのリストを入力および編集させるために使用するinput要素。電子メールアドレスを編集するための欄で、text 入力欄のように見えるものの、対応しているブラウザーや動的なキーボードのある端末では、入力値を検証したり、関連するキーボードを表示する。<br>&lt;input type='***' id='email' pattern='.+&commat;example\.com' size='30' required /&gt;`},
{target:`file`, content:`ユーザーがファイルを選択するコントロールのtype属性で指定するinputの型。accept 属性を使用して、コントロールが選択することができるファイル形式を定義することができる。<br>&lt;form method='post' enctype='multipart/form-data'&gt;<br>&emsp;&lt;div&gt;<br>&emsp;&emsp;&lt;label for='file'&gt;アップロードするファイルを選択&lt;/label&gt;<br>&emsp;&emsp;&lt;input type='***' id='file' name='file' multiple /&gt; → accept='image/*,.pdf'ならimageファイルとpdfファイル<br>&emsp;&lt;/div&gt;<br>&emsp;&lt;div&gt;<br>&emsp;&emsp;&lt;button&gt;送信&lt;/button&gt;<br>&emsp;&lt;/div&gt;<br>&lt;/form&gt;`},
{target:`hidden`, content:`表示されないコントロールですが、その値はサーバーに送信されるtype属性で指定するinputの型。隣の列には例がありますが、非表示。`},
{target:`image`, content:`テキストではなく画像の形をとる送信ボタンのためのtype属性で指定するinputの型。src 属性で定義された画像を表示。alt 属性は src の画像が見つからないときに表示。<br>&lt;input type='***' id='image' alt='Login' src='/media/login.png' /&gt;`},
...
{target:`number`, content:`数値を入力するためのコントロールのtype属性で指定するinputの型。スピナーを表示し、既定の検証を追加。動的キーボードを持つ一部の端末では、テンキーを表示。<br>&lt;input type='***' id='tentacles' name='tentacles' min='0' max='10' placeholder='1.0' step='0.01'/&gt;`},
{target:`password`, content:`入力値を隠す単一行テキストフィールドのtype属性で指定するinputの型。サイトが安全ではない場合はユーザーに警告。<br>&lt;input type='***' id='pass' name='password' minlength='8' required /&gt;`},
{target:`radio`, content:`ラジオボタンのtype属性で指定するinputの型で、同じ name の値を持つ複数の選択肢から一つの値を選択することができる。<br>&lt;form&gt;<br>&emsp;&lt;fieldset&gt;<br>&emsp;&emsp;&lt;legend&gt;希望連絡方法&lt;/legend&gt;<br>&emsp;&emsp;&lt;div&gt;<br>&emsp;&emsp;&emsp;&lt;input type='***' id='Choice1' name='contact' value='email' /&gt;<br>&emsp;&emsp;&emsp;&lt;label for='Choice1'&gt;メール&lt;/label&gt;<br>&emsp;&emsp;&emsp;&lt;input type='***' id='Choice2' name='contact' value='phone' /&gt;<br>&emsp;&emsp;&emsp;&lt;label for='Choice2'&gt;電話&lt;/label&gt;<br>&emsp;&emsp;&emsp;&lt;input type='***' id='Choice3' name='contact' value='mail' /&gt;<br>&emsp;&emsp;&emsp;&lt;label for='Choice3'&gt;郵便&lt;/label&gt;<br>&emsp;&emsp;&lt;/div&gt;<br>&emsp;&emsp;&lt;div&gt;<br>&emsp;&emsp;&emsp;&lt;button type='submit'&gt;送信&lt;/button&gt;<br>&emsp;&emsp;&lt;/div&gt;<br>&emsp;&lt;/fieldset&gt;<br>&lt;/form&gt;`},
{target:`range`, content:`厳密な値であることが重要ではない数値を入力するためのコントロールのtype属性で指定するinputの型。範囲のウィジェットを表示し、既定では中央の値。 min と max の組み合わせで、受け入れる値の範囲を定義することができる。<br>&lt;div&gt;<br>&emsp;&lt;input type='***' id='volume' name='volume' min='0' max='11' /&gt;<br>&emsp;&lt;label for='volume'&gt;Volume&lt;/label&gt;<br>&lt;/div&gt;`},
{target:`search`, content:`ユーザーが検索クエリを入力するために設計されたテキスト入力欄のtype属性で指定するinputの型。改行は自動的に取り除かれる。value 属性は、テキスト入力欄に入力された現在のテキストの値が入っている文字列。プロパティを使ってこれを受け取ることができる。q は検索入力に与えられる最も一般的な name で送信するとサーバーに送信されるデータの名前と値の組は q=searchtermとなる。<br>&lt;input type='***' id='mySearch' name='q' /&gt;<br>&lt;button&gt;Search&lt;/button&gt;<br>searchTerms = mySearch.value;`},
{target:`submit`, content:`フォームを送信するボタンのtype属性で指定するinputの型。 value 属性は、ボタンのラベルとして表示される文字列を示し、ボタンはその他の真の値を持たない。<br>&lt;input type='***' value='リクエストを送信' /&gt;`},
{target:`tel`, content:`電話番号を入力するためのコントロールのtype属性で指定するinputの型。動的なテンキーを備えた一部の機器では、電話用のテンキーを表示。<br>&lt;input type='***' id='phone' name='phone' pattern='[0-9]{3}-[0-9]{3}-[0-9]{4}' required /&gt;`},
...
{target:`url`, content:`URL を入力するための入力欄のtype属性で指定するinputの型。text 入力欄のように見えますが、対応しているブラウザーや動的なキーボードのある端末では、入力値を検証したり、関連するキーボードを表示。<br>&lt;input type='***' name='url' id='url' placeholder='https://example.com' pattern='https://.*' size='30' required /&gt;`},
{target:`week`, content:`年と週番号で構成される日付を入力するためのコントロールのtype属性で指定するinputの型。週番号はタイムゾーンを伴わない。<br>&lt;input type='week' name='week' id='camp-week' min='2024-W18' max='2024-W26' required /&gt;`},
...
{target:`&lt;picture&gt;`, content:`様々な画面や端末の条件に応じた画像を提供するHTML要素。0 個以上のsource要素と 1 つのimg要素を含む。ブラウザーは複数のsource子要素を検討し、その中から最も適切なものを選択。適切なものがない場合や、ブラウザーがこの要素に対応してない場合、img要素の src 属性で指定されたURLが選択される。<br>&lt;***&gt;<br>&emsp;&lt;source media='(max-width: 600px)' srcset='img1.jpg'&gt; → max-widthが600pxの場合（スマートフォンなど）画像表示<br>&emsp;&lt;img src='img2.jpg' alt=''&gt; → 条件に一致しない場合表示<br>&lt;/***&gt;`},
...
{target:`media`, content:`ユーザーエージェントがそれぞれのsource要素を評価するメディア条件を（メディアクエリーと同様に）指定するHTML要素pictureのsource要素の属性。<br>&emsp;&lt;source ***='(max-width: 600px)' srcset='img1.jpg'&gt; → max-widthが600pxの場合（スマートフォンなど）画像表示`},
...
{target:`loop`, content:`ビデオをループ再生するための属性。ビデオの再生が終了した際に、自動的に最初から再生を繰り返す。<br>&lt;video id='video' src='video.mp4' *** autoplay muted playsinline&gt;&lt;/video&gt; → 自動再生、ループ再生、音声ミュートで背景として表示`},
{target:`muted`, content:`動画をミュートにするか指定。<br>&lt;video ***&gt;<br>&emsp;&lt;source src='movie.mp4' type='video/mp4'&gt;<br>&lt;/video&gt;`},
{target:`playsinline`, content:`インライン再生を許可するための属性。モバイルデバイスの場合、ビデオが自動的にフルスクリーンで再生されるのを防ぐ。<br>&lt;video id='video' src='video.mp4' loop autoplay muted ***&gt;&lt;/video&gt; → 自動再生、ループ再生、音声ミュートで背景として表示`},
{target:`&lt;dl&gt;`, content:`HTMLの要素で説明リストを表す。一連の用語&lt;dt&gt;と説明&lt;dd&gt;をリスト化したもの。一般的な使用例として、用語集の作成やメタデータ（キーと値のペアのリスト）の表示など。<br>&lt;***&gt;<br>&emsp;&lt;dt&gt;著者&lt;/dt&gt;<br>&emsp;&lt;dd&gt;著者内容&lt;/dd&gt;<br>&emsp;&lt;dt&gt;出版社&lt;/dt&gt;<br>&emsp;&lt;dd&gt;出版社内容&lt;/dd&gt;<br>&lt;/***&gt;<br><br>*** {<br>&emsp;display: flex; → dt、ddを横並びにする<br>&emsp;flex-wrap: wrap; → dtとddが100%になったら、横並びを折り返す<br>}`},
{target:`&lt;dt&gt;`, content:`HTMLの要素で、説明または定義リストの中で用語を表す部分であり、&lt;dl&gt;の子要素としてのみ用いることができる。普通は&lt;dd&gt;要素が続く。複数の要素が続くと、すぐ後に続く&lt;dd&gt;要素で定義されていることを表す。後に続く&lt;dd&gt;（詳細説明）要素は、指定した用語について、定義やその他の関連する文字列を表す。<br>&lt;dl&gt;<br>&emsp;&lt;***&gt;著者&lt;/***&gt;<br>&emsp;&lt;dd&gt;著者内容&lt;/dd&gt;<br>&emsp;&lt;***&gt;出版社&lt;/***&gt;<br>&emsp;&lt;dd&gt;出版社内容&lt;/dd&gt;<br>&lt;/dl&gt;<br><br>dl {<br>&emsp;display: flex; → ***、ddを横並びにする<br>&emsp;flex-wrap: wrap; → ***とddが100%になったら、横並びを折り返す<br>}`},
...
{target:`&lt;th&gt;`, content:`表の内のヘッダーセル（見出しセル）を指定するHTML要素（Table Header Cell）。scope='col'はヘッダーセルが列の見出し、scope='row'はヘッダーセルが行の見出し、colspan='2'は横方向にいくつの列を結合するかを指定（ここでは2列）`},
{target:`&lt;td&gt;`, content:`表の内のデータセルを指定するHTML要素（Table Data Cell）。`},
{target:`&lt;aside&gt;`, content:`文書のメインコンテンツと間接的な関係しか持っていない文書の部分を表現するHTML要。サイドバーやコールアウトボックスなどを表現するのに使われる。<br>&lt;*** id='sidebar'&gt;&lt;/***&gt;<br>&lt;***&gt;<br>&emsp;&lt;p&gt;side&lt;/p&gt;<br>&lt;/***&gt;`},
...
{target:`calc`, content:`CSS のプロパティ値を指定する際に計算を行うことができるCSS関数。<br>width: ***(50% - 10px);<br>width: ***(100%/3); → widthの値が3等分になるよう計算`},
{target:`height`, content:`要素の高さを指定するCSSプロパティ。<br>***: 200px;`},
{target:`font-family`, content:`フォントの種類を指定するCSSプロパティ。選択した要素に対して、フォントファミリー名や総称ファミリー名の優先順位リストを指定することができる。<br>***: Arial',sans-serif; → 'Arial' フォントまたはサンセリフフォントを指定<br>***: 'Arial', 'Helvetica', sans-serif;`},
...
{target:`margin`, content:`要素の外側の余白を指定するCSSプロパティ。autoはブラウザーが適切なマージンを選択して使用し要素を中央揃えにする。<br>***: 10px 50px 20px 0;<br>***: 0 auto → 上は余白 0、左右はauto`},
{target:`display`, content:`HTML要素の表示を設定。noneは最初から無かったかのように要素を隠し、次の要素が繰り上がるCSSプロパティ。<br>***: none;&emsp;***: inline;`},
{target:`flex`, content:`要素は、ブロック要素のように動作しつつ、そのコンテンツをフレックスボックスモデルに従ってレイアウトするCSS値。水平方向に並ぶ。<br>display: ***;`},
{target:`grid`, content:`要素は、ブロックレベル要素のように動作しつつ、そのコンテンツをグリッドモデルに従ってレイアウトするCSS値。垂直方向に並ぶ。<br>display: ***;`},
...
{target:`inline-block`, content:`ブロック要素ボックスを生成するが、周囲のコンテンツに対しては単一のインラインボックスであるかのように振る舞うCSS値。幅と高さを持たせることができる。<br>display: ***;`},
...
{target:`flew-shrink`, content:`フレックスアイテムの縮小係数を設定するCSSプロパティ。すべてのフレックスアイテムの寸法がフレックスコンテナーよりも大きい場合、アイテムは数値に従い縮小して収まる。<br>***: 0; → 親コンテナが狭くなっても縮小しない<br>***: 1; → 親コンテナの余分なスペースが不足している場合に他のフレックスアイテムと同じ割合で縮小<br>***: 2; → 親コンテナの余分なスペースが不足している場合に他のフレックスアイテムの2倍縮小`},
...
{target:`stretch`, content:`各アイテムを均等に配置しサイズが auto であるアイテムはコンテナーに合わせて引き伸ばす。CSS値。<br>display: flex<br>justify-content: ***;`},
{target:`align-items`, content:`すべての直接の子要素に集合としてフレックスアイテムの垂直方向の配置を指定するCSSプロパティ。<br>***: stretch; → アイテムのマージンボックスの cross-size が、幅や高さの制約の範囲内でラインと同じになるよう拡張<br>***: center; → アイテムを垂直中央に配置<br>***: start; → アイテムを先頭にまとめる<br>***: end; → アイテムを末尾にまとめる`},
{target:`vertical-align`, content:`インラインボックス、インラインブロック、表セルボックスの垂直方向の配置を設定するCSSプロパティ。包含する行ボックスの中で、インライン要素のボックスの垂直方向の配置を決める場合と表のセルの内容の垂直方向の配置を決める場合に使用。<br>***: baseline; → 要素のベースラインを親要素のベースラインに揃える<br>***: top; → 要素と子孫要素の上端を行全体の上端に揃える<br>***: bottom; → 要素と子孫要素の下端を行全体の下端に揃える<br>***: middle; → 要素の中央を親要素の baseline + x-height の半分 に揃える<br>***: text-top; → 要素の上端を親要素のフォントの上端に揃える<br>***: sub; → 要素のベースラインを親要素の subscript-baseline に揃える`},
...
{target:`appearance`, content:`オペレーティングシステムのテーマに基づいた、プラットフォーム固有のスタイル設定で UI 要素を表示するために使用するCSSプロパティ。<br>***: auto; → ブラウザのデフォルトの外観を使用<br>***: menulist; → ドロップダウンリスト<br>***: menulist-box; → ドロップダウンメニューボックス<br>***: textfield → テキストフィールド<br>-moz-***: menulist; → Firefoxブラウザでドロップダウンリスト<br>-webkit-***: menulist; → WebKit系ブラウザ（ChromeやSafariなど）でドロップダウンリスト`},
...
{target:`background-position`, content:`背景画像の初期位置を設定するCSSプロパティ。位置は background-origin で設定された位置レイヤーからの相対。<br>***: center; → 中央揃え<br>***: 25% 75% → X方向に25%、Y方向に75%の位置`},
...
{target:`background-attachment`, content:`背景画像の位置をビューポートの中で固定するか、包含ブロックと一緒にスクロールするかを設定するCSSプロパティ。fixed：相対位置で固定。local：相対位置で固定。要素がスクロール機構を持っていた場合、背景画像は要素の内容とともにスクロール。scroll：背景は要素自身に対する相対位置で固定され、内容と共にスクロールしない。<br>***: scroll;<br>***: scroll, local;`},
...
{target:`border-radius`, content:`ボーダーの角の半径を指定するCSSプロパティ。50%を指定することで要素を円形にする。<br>***: 10px; → ボーダーの角を10pxの円で丸くする<br>***: 50%; → 要素を円形に指定`},
...
{target:`border-spacing`, content:`表における隣り合うセルの境界同士の間隔を定めるCSSプロパティ。このプロパティは border-collapse が separate のときのみ適用。<br>***: 5px 1rem; → 水平方向の間隔を5ピクセル、垂直方向の間隔を1リモート（ルート要素のフォントサイズと同じ）<br>***: 1cm 2em; → 水平方向の間隔を1センチメートル、垂直方向の間隔を2エメットリックス（要素のフォントサイズの2倍）`},
...
{target:`object-position`, content:`ボックス内における置換要素の中身の配置を指定するCSSプロパティ。ボックスの領域内で置換要素のオブジェクトに覆われていない部分は、要素の背景が表示。<br>***: center top; → 水平方向は中央、垂直方向はトップに配置`},
...
{target:`&commat;charset`, content:`スタイルシートで使う文字エンコーディングを定義するCSSアットルール。このルールはスタイルシートの最初の要素でなければならず、これより前には文字を一切記述してはいけない。<br>*** 'UTF-8';`},
{target:`&commat;media`, content:`1 つまたは複数のメディアクエリーの結果に基づいて、スタイルシートの一部を適用するCSSアットルール。「メディアタイプ」と「メディア特性」を指定することで、出力デバイスに応じてスタイルを適用したり、横幅に応じCSSを切り替えることができる。<br>*** screen and (max-width: 600px) {} → 画面サイズが小さいデバイス（例えばスマートフォン）に対し適切なスタイルへ切り替える`},
{target:`&commat;page`, content:`文書を印刷するときに一部の CSS プロパティを変更するために使用するCSSアットルール。<br>***{margin: 1cm;}`},
...
{target:`::before`, content:`選択した要素の最初の子要素として擬似要素を生成。よくcontentプロパティを使用して、要素に装飾的な内容を追加するために用いられる。<br>a *** {<br>&emsp;content: '♥';<br>} → リンクの前にハートマークを入れる`},
{target:`::after`, content:`選択した要素の最後の子要素として擬似要素を生成。よくcontentプロパティを使用して、要素に装飾的な内容を追加するために用いられる。<br>a *** {<br>&emsp;content: '→';<br>} → リンクの後に矢印を追加`},
{target:`-webkit-`, content:`基本的にWebKitベースのブラウザーすべてをさすベンダープレフィックス（Chrome、Safariなど）。ベンダープレフィックス付きのプロパティを使う場合は、ベンダープレフィックスが付いていないプロパティも併用して記述する。<br>#header {<br>&emsp;***justify-content: center;<br>&emsp;justify-content: center;<br>}`},
{target:`-moz-`, content:`Firefoxをさすベンダープレフィックス。ベンダープレフィックス付きのプロパティを使う場合は、ベンダープレフィックスが付いていないプロパティも併用して記述する。<br>#header {<br>&emsp;***justify-content: center;<br>&emsp;justify-content: center;<br>}`},
{target:`-ms-`, content:`IE、Edgeをさすベンダープレフィックス。ベンダープレフィックス付きのプロパティを使う場合は、ベンダープレフィックスが付いていないプロパティも併用して記述する。<br>#header {<br>&emsp;***justify-content: center;<br>&emsp;justify-content: center;<br>}`},
{target:`-o-`, content:`WebKit導入前の古いOperaのベンダープレフィックス。ベンダープレフィックス付きのプロパティを使う場合は、ベンダープレフィックスが付いていないプロパティも併用して記述する。<br>#header {<br>&emsp;***justify-content: center;<br>&emsp;justify-content: center;<br>}`},
{target:`twitter-timeline`, content:`Twitterのタイムラインを埋め込むためのリンク。class='***-***'は、このリンクがTwitterの埋め込みウィジェットであることを示す。data-height='315'は、埋め込まれたタイムラインの高さを指定。href='https://twitter.com/X?...'は、埋め込むTwitterアカウントのURLを示す。次の行にTwitterの埋め込みウィジェットを読み込むJavaScriptファイルへのリンクを指定する。<br>&lt;a class='***-***' data-height='315' href='https://twitter.com/X?ref_src=...'&gt;Tweets by X&lt;/a&gt;<br>&lt;script async src='https://platform.twitter.com/widgets.js' charset='utf-8'&gt;&lt;/script&gt;`},
