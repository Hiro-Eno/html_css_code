...
{target:`style`, content:`要素のインラインスタイルをCSS Style Declarationオブジェクトの形で返すHTML Elementの読み取り専用プロパティ。このオブジェクトは、その要素のインラインのstyle属性で定義されている属性に値が割り当てられ、その要素のすべてのスタイルプロパティのリストが入っている。スタイル宣言は、nullまたは空文字列を設定することでリセットできる。<br>&lt;body ***='font-weight:bold'&gt;<br>element.***.backgroundColor = 'red'<br>element.***.cssText = 'width: 300px; position: relative; top: 250px; left: 250px';`},
...
{target:`url`, content:`URLを入力するための入力欄のtype属性で指定するinputの型。text入力欄のように見えますが、対応しているブラウザーや動的なキーボードのある端末では、入力値を検証したり、関連するキーボードを表示。<br>&lt;input type='***' name='url' id='url' placeholder='https://example.com' pattern='https://.*' size='30' required /&gt;`},
