...
{target:`&lt;input&gt;`, content:`ユーザーからデータを受け取るための、ウェブベースのフォーム用の操作可能なコントロールを作成するためのHTML要素。この要素の動作はtype属性の値に応じて大きく異なる。入力した値はvalueプロパティで取得。<br>&lt;*** type='text' id='name' name='name' placeholder='Enter your username' required minlength='4' maxlength='8' size='10' /&gt;<br>const inputText = document.getElementByTagName('***').value;`},
...
{target:`email`, content:`ユーザーに一つのメールアドレス、または、multiple属性が設定されていた場合は、メールアドレスのリストを入力および編集させるために使用するinput要素。電子メールアドレスを編集するための欄で、text入力欄のように見えるものの、対応しているブラウザーや動的なキーボードのある端末では、入力値を検証したり、関連するキーボードを表示する。<br>&lt;input type='***' id='email' pattern='.+&commat;example\.com' size='30' required /&gt;`},
{target:`file`, content:`ユーザーがファイルを選択するコントロールのtype属性で指定するinputの型。accept属性を使用して、コントロールが選択することができるファイル形式を定義することができる。<br>&lt;form method='post' enctype='multipart/form-data'&gt;<br>&emsp;&lt;div&gt;<br>&emsp;&emsp;&lt;label for='file'&gt;アップロードするファイルを選択&lt;/label&gt;<br>&emsp;&emsp;&lt;input type='***' id='file' name='file' multiple /&gt; → accept='image/*,.pdf'ならimageファイルとpdfファイル<br>&emsp;&lt;/div&gt;<br>&emsp;&lt;div&gt;<br>&emsp;&emsp;&lt;button&gt;送信&lt;/button&gt;<br>&emsp;&lt;/div&gt;<br>&lt;/form&gt;`},
{target:`hidden`, content:`表示されないコントロールですが、その値はサーバーに送信されるtype属性で指定するinputの型。隣の列には例がありますが、非表示。`},
{target:`image`, content:`テキストではなく画像の形をとる送信ボタンのためのtype属性で指定するinputの型。src属性で定義された画像を表示。alt属性はsrcの画像が見つからないときに表示。<br>&lt;input type='***' id='image' alt='Login' src='/media/login.png' /&gt;`},
...
{target:`range`, content:`厳密な値であることが重要ではない数値を入力するためのコントロールのtype属性で指定するinputの型。範囲のウィジェットを表示し、既定では中央の値。minとmaxの組み合わせで、受け入れる値の範囲を定義することができる。<br>&lt;div&gt;<br>&emsp;&lt;input type='***' id='volume' name='volume' min='0' max='11' /&gt;<br>&emsp;&lt;label for='volume'&gt;Volume&lt;/label&gt;<br>&lt;/div&gt;`},
{target:`search`, content:`ユーザーが検索クエリを入力するために設計されたテキスト入力欄のtype属性で指定するinputの型。改行は自動的に取り除かれる。value属性は、テキスト入力欄に入力された現在のテキストの値が入っている文字列。プロパティを使ってこれを受け取ることができる。qは検索入力に与えられる最も一般的なnameで送信するとサーバーに送信されるデータの名前と値の組は q=searchtermとなる。<br>&lt;input type='***' id='mySearch' name='q' /&gt;<br>&lt;button&gt;Search&lt;/button&gt;<br>searchTerms = mySearch.value;`},
{target:`submit`, content:`フォームを送信するボタンのtype属性で指定するinputの型。value属性は、ボタンのラベルとして表示される文字列を示し、ボタンはその他の真の値を持たない。<br>&lt;input type='***' value='リクエストを送信' /&gt;`},
...
{target:`url`, content:`URL を入力するための入力欄のtype属性で指定するinputの型。text入力欄のように見えますが、対応しているブラウザーや動的なキーボードのある端末では、入力値を検証したり、関連するキーボードを表示。<br>&lt;input type='***' name='url' id='url' placeholder='https://example.com' pattern='https://.*' size='30' required /&gt;`},
...
{target:`&lt;time&gt;`, content:`特定の時の区間を表します。datetime属性を使用して、機械可読な形式の日付を記述することができ、検索エンジンの結果を改善したりリマインダーなどの独自機能に使用できる。<br>&lt;*** datetime='2022-01-04'&gt;...&lt;/time&gt;`},
...
{target:`method`, content:`フォームを送信する際にブラウザーが使用するHTTPメソッド。postを指定することでフォームのデータはリクエスト本体として送信。getを指定することでフォームのデータはaction属性のURLに?で区切って追加される。<br>&lt;form action='save_data.php' ***='post' class='form-example'&gt;`},
...
{target:`&lt;aside&gt;`, content:`文書のメインコンテンツと間接的な関係しか持っていない文書の部分を表現するHTML要素。サイドバーやコールアウトボックスなどを表現するのに使われる。<br>&lt;*** id='sidebar'&gt;&lt;/***&gt;<br>&lt;***&gt;<br>&emsp;&lt;p&gt;side&lt;/p&gt;<br>&lt;/***&gt;`},
...
{target:`relative`, content:`相対位置指定。要素は文書の通常のフローに従って配置され、top、right、bottom、leftの値に基づいて自分自身からの相対オフセットで配置するCSS値。<br>position: ***;`},
{target:`sticky`, content:`固定位置指定（親基準）。要素は文書の通常のフローに従って配置され、直近のスクロールする祖先および包含ブロック（直近のブロックレベル祖先、表関連要素を含む）に対してtop、right、bottom、leftの値に基づいて相対配置するCSS値。<br>position: ***;`},
...
{target:`background-position`, content:`背景画像の初期位置を設定するCSSプロパティ。位置はbackground-originで設定された位置レイヤーからの相対。<br>***: center; → 中央揃え<br>***: 25% 75% → X方向に25%、Y方向に75%の位置`},
...
{target:`border-spacing`, content:`表における隣り合うセルの境界同士の間隔を定めるCSSプロパティ。このプロパティはborder-collapseがseparateのときのみ適用。<br>***: 5px 1rem; → 水平方向の間隔を5ピクセル、垂直方向の間隔を1リモート（ルート要素のフォントサイズと同じ）<br>***: 1cm 2em; → 水平方向の間隔を1センチメートル、垂直方向の間隔を2エメットリックス（要素のフォントサイズの2倍）`},
...
{target:`contain`, content:`ある要素とその内容が、できる限り多く、文書ツリーの他の部分から独立していることを示す。これによってブラウザーがレイアウト、スタイル、描画、寸法、およびその組み合わせの再計算を、ページ全体ではなくDOMの限られた領域に対して行うことで、性能上の明らかな利点をもたらす。このプロパティはページ上にそれぞれ独立したたくさんのウィジェットがあるときに有益であり、ウィジェットの内部を、ウィジェットの囲みボックスの外側の副作用から守るために使用することができるCSSプロパティ。<br>***: paint;`},
...
{target:`flex-basis`, content:`フレックスアイテムの主要部分の初期の寸法を設定。box-sizingで設定していない限り、このプロパティはコンテンツボックスの寸法を定義するCSSプロパティ。<br>***: 50%;`},
