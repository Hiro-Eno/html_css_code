...
{target:`&commat;media`, content:`1 つまたは複数のメディアクエリーの結果に基づいて、スタイルシートの一部を適用するCSSアットルール。<br>「メディアタイプ」と「メディア特性」を指定することで、出力デバイスに応じてスタイルを適用したり、横幅に応じCSSを切り替えることができる。<br>*** screen and (max-width: 600px) {} → 画面サイズが小さいデバイス（例えばスマートフォン）に対し適切なスタイルへ切り替える`},
