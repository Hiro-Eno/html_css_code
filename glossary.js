...
{target:`&lt;iframe&gt;`, content:`入れ子になった閲覧コンテキストを表現し、現在の HTML ページに他のページを埋め込むことができるHTML要素。src属性で埋め込むコンテンツのURL、width属性とheight属性で埋め込まれたコンテンツの幅と高さ、allowfullscreen属性でフルスクリーン表示を許可を指定。loading属性でブラウザがコンテンツを読み込む方法を指定し、lazy値は、コンテンツが必要になるまで読み込まない。style属性はインラインCSSスタイルが含まれ、border:0;では周囲に枠線が表示されない。<br>&lt;iframe src='https://www.***' width='600' height='450' style='border:0;' allowfullscreen='true' loading='lazy'&gt;&lt;/iframe&gt;`},
