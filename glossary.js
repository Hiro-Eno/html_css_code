...
{target:`width`, content:`要素の幅を指定するCSSプロパティ。<br>***: 200px;<br>display: flex;<br>***: calc(100%/3); → 値が3等分になるよう計算`},
...
{target:`&commat;media`, content:`CSSのメディアクエリのあとに「メディアタイプ」と「メディア特性」を指定することで、出力デバイスに応じてスタイルを適用したり、横幅に応じCSSを切り替えることができる。<br>*** screen and (max-width: 600px) {} → 画面サイズが小さいデバイス（例えばスマートフォン）に対し適切なスタイルへ切り替える`},
]
